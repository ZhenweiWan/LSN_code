...
class TSNDataSet_3D(data.Dataset):
    ...
    def get(self, record, indices):
        ...
        for p in process_idx_list:
            # seg_imgs = self._load_image(os.path.join('/media/icy/zbb/DIVING48/Diving48_frames' + '/'+record.path.strip().split('/')[-1]), p)
            # seg_imgs = self._load_image(os.path.join('/media/icy/zbb/sth-v2/20bn-something-something-v2-frames' + '/'+record.path.strip().split('/')[-1]), p)
            # seg_imgs = self._load_image(os.path.join('/media/icy/zbb/K400_frames_256edge/train_val' + '/'+ record.path.strip().split('/')[0] +'/'+record.path.strip().split('/')[-1]), p)
            seg_imgs = self._load_image(os.path.join(
                '/home/vv/dataset/something/v1/20bn-something-something-v1/something/v1/img' + '/' +
                record.path.strip().split('/')[-1]), p)
